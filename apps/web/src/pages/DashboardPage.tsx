...
const s: Record<string, React.CSSProperties> = {
  ...
  kpi:     { background: "#fff", borderRadius: 10, border: "1px solid #E5E7EB", padding: "18px 20px", display: "flex", justifyContent: "space-between", alignItems: "flex-start" },
  kpiL:    { fontSize: 12, color: "#6B7280", marginBottom: 6 },
  kpiV:    { fontSize: 26, fontWeight: 700, color: "#111827" },
  ...
  card:    { background: "#fff", borderRadius: 10, border: "1px solid #E5E7EB", padding: "20px" },
  cardH:   { fontSize: 14, fontWeight: 600, color: "#111827", marginBottom: 16 },
  skel:    { background: "#F3F4F6", borderRadius: 8, animation: "pulse 1.5s ease-in-out infinite" },
...
export default function DashboardPage() {
  ...
  const KPIS = [
    { label: "Toplam Tesis",    value: loading ? "—" : String(installations.length),          icon: "🏭", iconBg: "#EFF6FF", iconColor: "#1D4ED8" },
    ...
    return (
      <div style={s.page}>
        ...
        <div style={s.empty}>
          ...
          <Link to="/cbam" style={{ color: "#0066CC", fontSize: 14 }}>→ Tesis Ekle</Link>
        </div>
      </div>
    );
  }
      ...
      <div style={s.row2}>
        ...
        <div style={s.card}>
          ...
          {loading ? (
            ...
            <div style={{ color: "#9CA3AF", fontSize: 13 }}>Veri yok</div>
          ) : (
        ...
        <div style={s.card}>
          ...
          {loading ? (
            ...
            <div style={{ color: "#9CA3AF", fontSize: 13 }}>Hesaplanmış dönem yok</div>
          ) : (
            <ResponsiveContainer width="100%" height={240}>
              <BarChart data={seeData} margin={{ top: 5, right: 10, bottom: 40, left: 0 }}>
                ...
                <Legend />
                <Bar dataKey="seeBaseline" name="Baseline" fill="#E5E7EB" />
            ...
            <thead>
              <tr>
                {["Tesis", "Dönem", "SEE Baseline", "SEE Voltfox", "Azaltım", "CBAM Tasarruf"].map(h => (
                  <th key={h} style={{ padding: "8px 12px", textAlign: "left", fontSize: 12, fontWeight: 600, color: "#6B7280", borderBottom: "1px solid #E5E7EB", background: "#F9FAFB" }}>{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {withResult.slice(0, 10).map((p, i) => (
                <tr key={i} style={{ borderBottom: "1px solid #F3F4F6" }}>
                  ...
                  <td style={{ padding: "10px 12px", fontSize: 13, color: "#6B7280" }}>{p.periodName}</td>
